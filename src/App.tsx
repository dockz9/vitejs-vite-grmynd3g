...
function ContactsTab({ contacts, emails, meetings }) {
  ...
  async function save() {
    ...
    if (editing) {
      await col.update(editing.id, data);
    } else {
      await col.add(data);
    }
    setShowModal(false);
  ...
  return (
    <div>
      ...
      >
        ...
        >
          {['all', 'prospect', 'active', 'customer', 'inactive'].map((s) => (
          ...
          return (
            ...
            >
              ...
              >
                <div>
                  <div
                    style={{ fontSize: 18, fontWeight: 700, color: '#9999cc' }}
                  >
                    {cEmails}
                  </div>
                  emails
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <Btn size="sm" variant="ghost" onClick={() => openEdit(c)}>
                  Edit
                </Btn>
                <Btn
                  size="sm"
                  variant="danger"
                  onClick={() => col.remove(c.id)}
                >
                  ×
                </Btn>
              </div>
      ...
      {showModal && (
        ...
        >
          ...
          <Sel
            ...
            options={['prospect', 'active', 'customer', 'inactive'].map(
              (s) => ({
                value: s,
          ...
          <Field
            ...
            placeholder="e.g. enterprise, warm lead"
          />
          <Field
            label="Notes"
            value={form.notes}
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            ...
            <Btn onClick={save} disabled={!form.name || !form.email}>
              Save Contact
            </Btn>
          </div>
        </Modal>
        ...
        {sorted.map((e) => {
          ...
          return (
            ...
            >
                ...
                >
                  ...
                  <span
                    style={{
                      ...
                      fontWeight: 600,
                      background:
                        e.direction === 'sent' ? '#6366f120' : '#10b98120',
                      color: e.direction === 'sent' ? '#6366f1' : '#10b981',
                      border: `1px solid ${
                        e.direction === 'sent' ? '#6366f140' : '#10b98140'
                      }`,
                    }}
                  >
                    {e.direction}
                  ...
                  <span
                    style={{ fontSize: 11, color: '#555', marginLeft: 'auto' }}
                  >
                    {e.date}
                  </span>
                </div>
                <div style={{ fontSize: 13, color: '#888', marginBottom: 4 }}>
                  {contact ? `${contact.name} · ${contact.company}` : 'Unknown'}
                </div>
                <div
              ...
              <Btn size="sm" variant="danger" onClick={() => col.remove(e.id)}>
                ×
              </Btn>
            </div>
          );
      ...
      {showModal && (
        <Modal title="Log Email" onClose={() => setShowModal(false)}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            ...
            <Btn onClick={save} disabled={!form.subject}>
              Save Email
            </Btn>
          </div>
        </Modal>
...
function MeetingsTab({ meetings, contacts }) {
  ...
  function MeetingCard({ m }) {
    ...
    const dateStr = m.date
      ? `${m.date.slice(5, 7)}/${m.date.slice(8, 10)}`
      : '--';
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {m.status === 'upcoming' && (
            <Btn
              size="sm"
              variant="green"
              onClick={() => col.update(m.id, { status: 'completed' })}
            >
              ✓ Done
            </Btn>
          )}
          <Btn size="sm" variant="danger" onClick={() => col.remove(m.id)}>
            ×
          </Btn>
        </div>
      </div>
      ...
      {showModal && (
        <Modal title="Schedule Meeting" onClose={() => setShowModal(false)}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            ...
            <Btn onClick={save} disabled={!form.title || !form.date}>
              Schedule
            </Btn>
          </div>
        </Modal>
...
function OutreachTab({ contacts }) {
  ...
  useEffect(() => {
    if (prospects.length && !selected) setSelected(prospects[0].id);
  }, [prospects]);
      ...
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'user',
              content: `Write a cold outreach email to a prospect. Return ONLY a JSON object (no markdown, no backticks) with keys "subject" and "body".\n\nProspect: ${
                contact.name
    ...
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
      ...
      >
        ...
        <p style={{ margin: 0, fontSize: 13, color: '#888' }}>
          Generate personalized cold emails — logged to the cloud instantly.
        </p>
      </div>

      {prospects.length === 0 ? (
        <div style={{ textAlign: 'center', color: '#555', padding: '60px 0' }}>
          <div style={{ fontSize: 32, marginBottom: 12 }}>📭</div>Add contacts
          with "prospect" status to get started.
        </div>
      ) : (
        <div style={{ display: 'grid', gap: 16 }}>
          ...
          {contact && (
            ...
            >
              ...
              <div>
                <div style={{ fontWeight: 700, color: '#f0f0ff' }}>
                  {contact.name}
                </div>
          ...
          />
          <Btn onClick={generate} disabled={loading || !contact}>
          ...
          {result && (
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <Btn variant="green" onClick={markSent}>
                  ✓ Mark as Sent
                </Btn>
                  ...
                  .map((s, i) => (
                    ...
                    >
                      ...
                      <span style={{ fontSize: 13, color: '#ccc' }}>
                        <strong>{s.contactName}</strong> — {s.subject}
                      </span>
                      <span
                        style={{
                          marginLeft: 'auto',
...
export default function CRM() {
  ...
  const loading =
    contactsCol.loading || emailsCol.loading || meetingsCol.loading;

  const stats = [
    { label: 'Contacts', value: contactsCol.docs.length, color: '#6366f1' },
    {
      label: 'Prospects',
      value: contactsCol.docs.filter((c) => c.status === 'prospect').length,
      color: '#f59e0b',
    },
    {
      label: 'Customers',
      value: contactsCol.docs.filter((c) => c.status === 'customer').length,
      color: '#10b981',
    },
    {
      label: 'Upcoming',
      value: meetingsCol.docs.filter((m) => m.status === 'upcoming').length,
      color: '#3b82f6',
    },
  ];
      ...
      >
        ...
        <div style={{ padding: '24px', maxWidth: 1100, margin: '0 auto' }}>
          {firebaseConfig.apiKey === 'PASTE_YOUR_API_KEY' && (
            ...
            >
              ⚠️ <strong>Setup required:</strong> Open this file in StackBlitz
              and replace the <code>firebaseConfig</code> values at the top with
              your Firebase project credentials. See the instructions in the
              chat.
            </div>
          )}
              ...
              >
                ...
                {t === 'Outreach' && (
                  <span
                    style={{
                      marginLeft: 6,
                      background: tab === t ? '#ffffff30' : '#6366f130',
          ...
          <div style={{ animation: 'fadeIn 0.2s ease' }} key={tab}>
            {loading ? (
              ...
              <>
                {tab === 'Contacts' && (
                  <ContactsTab
                    contacts={contactsCol.docs}
                    emails={emailsCol.docs}
                    meetings={meetingsCol.docs}
                  />
                )}
                {tab === 'Emails' && (
                  <EmailsTab
                ...
                {tab === 'Outreach' && (
                  <OutreachTab contacts={contactsCol.docs} />
                )}
              </>
            )}
          </div>
